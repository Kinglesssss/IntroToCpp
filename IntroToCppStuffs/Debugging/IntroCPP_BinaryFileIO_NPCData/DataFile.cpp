...
//setting up the record index
DataFile::Record* DataFile::GetRecord(int index, string filename)
{
	ifstream infile(filename, ios::binary);

	infile.seekg(recordSizes[index], ios::beg);
...
//loading in the npc information to use
void DataFile::Load(string filename)
{
	...
	for (int i = 0; i < recordCount; i++)
	{
		recordSizes.push_back(infile.tellg());
...
//clearing the load so you can change npc
void DataFile::Clear()
{
